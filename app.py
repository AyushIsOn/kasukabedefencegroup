...
if st.sidebar.button('Calculate Repayment Plan'):
    ...
    for year, principal_payment, interest_payment, remaining_debt in repayment_plan:
        ...
        predic_inflation = model.predict(user_data)
        st.write('Last year`s Inflation Prediction:', predic_inflation[0])
        
        # Retrieve inflation for the previous year
        prev_year_data = np.array([[year - 1]])
        prev_year_inflation = model.predict(prev_year_data)[0]
        
        # Compare inflation with the previous year and make recommendation
        if predic_inflation[0] > prev_year_inflation:
            st.write("Recommendation: It's recommended to increase gold's rate, since the inflation this year is higher than the previous year.")
        else:
            st.write("Recommendation: It's recommended to increase BSE's rate, since the inflation this year is less than the previous year.")
